...
string preRoundTransform(string x, string y){
    string res = "";
...
int main(){
    ...
    cout << "\nEnter first 64-bit text in hexadecimal(16-digits) : " ;
    cin >> hex >> hexkey1; 

    cout << "\nEnter next 64-bit text in hexadecimal(16-digits) : " ;
    cin >> hex >> hexkey2; 

    string text = bitset<64>(hexkey1).to_string() + bitset<64>(hexkey2).to_string();

	string temp = preRoundTransform(key, text);

    // create state matrix
    int k=0;
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
            state[j][i] = temp.substr(k*8, 8);
            k++;
        }
    }
